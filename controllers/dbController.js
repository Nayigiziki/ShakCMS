...
var saveAboutToDb = function(aboutObj, cb){
  var newAbout = new AboutModel({aboutContent: aboutObj.content, links : aboutObj.links});
...
var editAbout = function(id, aboutObj, cb){
    AboutModel.update({ _id: id }, { $set: {aboutContent: aboutObj.aboutContent, links: aboutObj.links}}, function(err, abt){
    if(err){
      console.log('err ' , err);
    } else {
...
var saveContactToDb = function(contactObj, cb){
  var newContact = new ContactModel({contactContent: contactObj.contactContent});
...
var editContact = function(id, contactObj, cb){
    ContactModel.update({ _id: id }, { $set: {contactContent: contactObj.contactContent}}, function(err, contact){
    if(err){
      console.log('err ' , err);
    } else {
