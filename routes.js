...
var setup = function(app) {
  ...
  app.post('/api/about', function(req, res, next){
    ...
    db.getAboutFromDB(function(err, about){
      ...
      if(about.length === 0){
        ...
        db.saveAboutToDb({content: abt.aboutContent, links: abt.links}, function(err, aboutObj){
          console.log('err ', err);
          if(!err){
            res.status(201).json({status: 'successfully created About', about: aboutObj});
          } else {
            res.status(400).json({status: 'unsuccessfully created About', err: err});
          }
        })
      }
      ...
      if(about.length === 1){
        ...
        var id = about[0]._id;
        db.editAbout(id, {aboutContent: abt.aboutContent, links: abt.links}, function(err,aboutObj){
          if(!err){
            res.status(201).json({status: 'successfully updated About', about: aboutObj});
          } else {
            res.status(400).json({status: 'unsuccessfully updated About', err: err});
          }
        })
  ...
  app.get('/api/about', function(req, res, next){
    db.getAboutFromDB(function(err, about){
      if(!err){
        res.status(200).json({status: 'successfully fetched About', about: about});
      } else {
        res.status(400).json({status: 'unsuccessfully fetched About', err: err});
      }
    })
  })
  ...
  app.post('/api/contact', function(req, res, next){
    ...
    db.getContactFromDB(function(err, contact){
      ...
      if(contact.length === 0){
        ...
        db.saveContactToDb({contactContent: contactInfo.contactContent}, function(err, contactObj){
          console.log('err ', err);
          if(!err){
            res.status(201).json({status: 'successfully created contact', contact: contactObj});
          } else {
            res.status(400).json({status: 'unsuccessfully created contact', err: err});
          }
        })
      }
      ...
      if(contact.length === 1){
        ...
        db.editContact(id, {contactContent: contactInfo.contactContent}, function(err,contactObj){
          if(!err){
            res.status(201).json({status: 'successfully updated contact', contact: contactObj});
          } else {
            res.status(400).json({status: 'unsuccessfully updated contact', err: err});
          }
        })
  ...
  app.get('/api/contact', function(req, res, next){
    db.getContactFromDB(function(err, contact){
      if(!err){
        res.status(200).json({status: 'successfully fetched contact', contact: contact});
      } else {
        res.status(400).json({status: 'unsuccessfully fetched contact', err: err});
      }
    })
  })
